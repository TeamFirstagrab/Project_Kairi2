...
public class PlayerMovement : MonoBehaviour
{
	...
	private float dashSpeed = 12f;	// 대쉬 속도
	...
	private void Update()
	{
		moveSpeed = GameManager.Instance.playerStatsRuntime.speed;
	...
	private void Move()	// 플레이어 이동
	{
		...
		if(dashRequested && isCrouching && hasHorizontalInput && !isDash)
		{
			...
			Vector2 rayOrigin = (Vector2)collider.bounds.center;    // 발 아래 Raycast 방향
			float sniffDist = collider.bounds.extents.y + 0.8f;
			...
			currDashVelocity = dirVec * dashSpeed;
			return;
		}
		...
		// 오르막에서 붕 뜸 방지
		else if(groundChecker.isGrounded && velY > 0.05f)
		{
	...
	private void ApplyGravityModifiers()
	{
		if(groundChecker.isGrounded)
		{
			...
			float targetSpeed = isCrouchPressed ? 0 : (inputVec.x * moveSpeed);
